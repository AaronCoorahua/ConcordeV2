...
export default function SalaBlockPage(): JSX.Element {
  const source = readSrc("src/blocks/Sala/Sala.tsx");
    ...
    <div style={{ minHeight: "100vh", background: "#ffffff", color: "#0f172a", fontFamily: "var(--vmc-font-display, 'Plus Jakarta Sans', -apple-system, sans-serif)" }}>
      ...
      <section style={{ maxWidth: 1120, margin: "0 auto", padding: "8px 40px 80px" }}>
        ...
        <CodeViewer
          ...
          filename="terminal"
          note={`O por URL: npx @subastop/concorde@latest add ${BASE_URL}/r/sala.json · Cae en concorde/blocks/Sala/.`}
        />

        <h2 style={{ fontSize: 18, fontWeight: 800, color: "#0f172a", margin: "0 0 6px" }}>Código del bloque</h2>
        <p style={{ fontSize: 13, color: "#64748b", margin: "0 0 18px" }}>
          Lienzo base del bloque (por ahora solo el fondo #2E0F70 — iremos montando las secciones).
        </p>
        <CodeViewer code={source} filename="src/blocks/Sala/Sala.tsx" note="Lienzo 1023×1042." />
      </section>
    </div>
  );
}
