/**
 * /handoff/businessicon — Documentación de BusinessIcon
 */
...
function readComponentSource(): string {
  try {
    return readFileSync(join(process.cwd(), "src/components/BusinessIcon.tsx"), "utf8");
  } catch {
    return "// No se pudo leer BusinessIcon.tsx en build.";
  }
}

const USAGE = `import BusinessIcon from "@/src/components/BusinessIcon";

// Autónomo — trackea hover/active solo
<BusinessIcon size={24} />

// Controlado — el padre pasa el estado
<BusinessIcon size={24} state="hover" />`;
...
const EXAMPLES: Example[] = [
  {
    id: "states",
    title: "Estados",
    description: "Default (gradiente púrpura diagonal en stroke), hover (gradiente naranja), active (blanco sólido). Icono stroke-based — strokeWidth 1.5px.",
    node: (
      <div style={DARK}>
        <div style={COL}><BusinessIcon size={28} state="default" /><span style={LABEL}>default</span></div>
        <div style={COL}><BusinessIcon size={28} state="hover"   /><span style={LABEL}>hover</span></div>
        <div style={COL}><BusinessIcon size={28} state="active"  /><span style={LABEL}>active</span></div>
      </div>
    ),
    code: `<BusinessIcon size={28} state="default" />
<BusinessIcon size={28} state="hover" />
<BusinessIcon size={28} state="active" />`,
  },
  {
    id: "sizes",
    title: "Tamaños",
...
export default function BusinessIconHandoffPage(): JSX.Element {
  ...
  return (
    <main style={{ maxWidth: 768, margin: "0 auto", padding: "48px 24px 96px", fontFamily: "var(--vmc-font-display,'Plus Jakarta Sans',-apple-system,sans-serif)", color: "#09090b" }}>
      ...
      <h1 style={{ fontSize: 30, fontWeight: 800, letterSpacing: "-0.02em", margin: "0 0 8px" }}>BusinessIcon</h1>
      <p style={{ ...muted, fontSize: 16 }}>Icono de edificio / negocio para la navegación lateral del sidebar. Stroke-based (1.5px, round caps).</p>

      <div style={{ marginTop: 28 }}>
        <Preview minHeight={140} code={`<BusinessIcon size={32} state="default" />\n<BusinessIcon size={32} state="hover" />\n<BusinessIcon size={32} state="active" />`}>
          <div style={DARK}>
            <BusinessIcon size={32} state="default" />
            <BusinessIcon size={32} state="hover" />
            <BusinessIcon size={32} state="active" />
          </div>
        </Preview>
      ...
      <InstallCommand name="businessicon" />
      ...
      <details>
        <summary style={{ cursor: "pointer", fontSize: 14, color: "#52525b", padding: "10px 14px", border: "1px solid #e4e4e7", borderRadius: 10, background: "#fafafa", userSelect: "none", listStyle: "none", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span>Ver <code style={{ fontFamily: "ui-monospace,monospace", fontSize: 13 }}>BusinessIcon.tsx</code> completo · self-contained · zero deps</span>
          <span style={{ color: "#a1a1aa" }}>▾</span>
        </summary>
        <div style={{ marginTop: 12 }}>
          <CodeBlock code={source} filename="BusinessIcon.tsx" maxHeight={520} />
        </div>
      </details>
    </main>
  );
