/**
 * /handoff/offericon — Documentación de OfferIcon
 */
...
function readComponentSource(): string {
  try {
    return readFileSync(join(process.cwd(), "src/components/OfferIcon.tsx"), "utf8");
  } catch {
    return "// No se pudo leer OfferIcon.tsx en build.";
  }
}

const USAGE = `import OfferIcon from "@/src/components/OfferIcon";

// Autónomo — trackea hover/active solo
<OfferIcon size={24} />

// Controlado — el padre pasa el estado
<OfferIcon size={24} state="hover" />`;
...
const EXAMPLES: Example[] = [
  {
    ...
    node: (
      <div style={DARK}>
        <div style={COL}><OfferIcon size={28} state="default" /><span style={LABEL}>default</span></div>
        <div style={COL}><OfferIcon size={28} state="hover"   /><span style={LABEL}>hover</span></div>
        <div style={COL}><OfferIcon size={28} state="active"  /><span style={LABEL}>active</span></div>
      </div>
    ),
    code: `<OfferIcon size={28} state="default" />
<OfferIcon size={28} state="hover" />
<OfferIcon size={28} state="active" />`,
  },
  {
    id: "sizes",
    title: "Tamaños",
...
export default function OfferIconHandoffPage(): JSX.Element {
  ...
  return (
    <main style={{ maxWidth: 768, margin: "0 auto", padding: "48px 24px 96px", fontFamily: "var(--vmc-font-display,'Plus Jakarta Sans',-apple-system,sans-serif)", color: "#09090b" }}>
      ...
      <h1 style={{ fontSize: 30, fontWeight: 800, letterSpacing: "-0.02em", margin: "0 0 8px" }}>OfferIcon</h1>
      <p style={{ ...muted, fontSize: 16 }}>Icono de oferta / subasta (mano + sobres de puja) para la navegación lateral del sidebar.</p>

      <div style={{ marginTop: 28 }}>
        <Preview minHeight={140} code={`<OfferIcon size={32} state="default" />\n<OfferIcon size={32} state="hover" />\n<OfferIcon size={32} state="active" />`}>
          <div style={DARK}>
            <OfferIcon size={32} state="default" />
            <OfferIcon size={32} state="hover" />
            <OfferIcon size={32} state="active" />
          </div>
        </Preview>
      ...
      <details>
        <summary style={{ cursor: "pointer", fontSize: 14, color: "#52525b", padding: "10px 14px", border: "1px solid #e4e4e7", borderRadius: 10, background: "#fafafa", userSelect: "none", listStyle: "none", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span>Ver <code style={{ fontFamily: "ui-monospace,monospace", fontSize: 13 }}>OfferIcon.tsx</code> completo · self-contained · zero deps</span>
          <span style={{ color: "#a1a1aa" }}>▾</span>
        </summary>
        <div style={{ marginTop: 12 }}>
          <CodeBlock code={source} filename="OfferIcon.tsx" maxHeight={520} />
        </div>
      </details>
    </main>
  );
